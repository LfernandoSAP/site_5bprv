            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200">
                        ...
                        <th class="text-left py-3 px-2 font-semibold text-[#6e6e6e]">Destaque</th>
                ...
                <tbody>
                    @forelse ($posts as $post)
                        <tr class="border-b border-gray-100 hover:bg-gray-50/50">
                            <td class="py-3 px-2">
                                ...
                                <div class="text-sm text-[#6e6e6e]">{{ $post->slug }}</div>
                            </td>
                            <td class="py-3 px-2">
                            ...
                            <td class="py-3 px-2 text-sm text-[#6e6e6e]">{{ optional($post->published_at)->format('d/m/Y H:i') ?? 'Não publicada' }}</td>
                            <td class="py-3 px-2 text-sm">{{ $post->is_featured ? 'Sim' : 'Não' }}</td>
                            <td class="py-3 px-2 text-right">
                                <div class="inline-flex gap-2">
                                    <a href="{{ ag('posts/' . $post->slug . '/edit') }}" class="px-3 py-1.5 text-sm border border-[#101010]/18 text-[#101010] rounded-full hover:bg-[#101010] hover:text-white transition">Editar</a>
                                    <form method="POST" action="{{ ag('posts/' . $post->slug) }}" onsubmit="return confirm('Deseja remover esta notícia?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-12 text-center text-[#6e6e6e]">Nenhuma notícia cadastrada ainda.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
